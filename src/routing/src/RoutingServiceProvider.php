...
class RoutingServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->singleton(UrlMatcher::class, static fn (App $app): UrlMatcher => new UrlMatcher($app['routing.collection'], $app['routing.request.context']));
        $this->app->alias(UrlMatcher::class, 'routing.url.matcher');

        $this->app->singleton(Router::class, static fn (App $app): Router => new Router(new Route(), $app['routing.collection']));
    ...
    {
        $this->app['event.dispatcher']->addSubscriber(new RouterListener($this->app['routing.url.matcher'], $this->app['http_kernel.request_stack']));
    }
}
